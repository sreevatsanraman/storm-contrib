...
public class StatefulTransactionalBoltExecutor<T extends State> implements ICommitter, IBatchBolt<TransactionAttempt> {
    public static String STATE_RESOURCE = StatefulTransactionalBoltExecutor.class.getName() + "/state";
    
    IStatefulTransactionalBolt<T> _delegate;
    T _state;
    TransactionAttempt _attempt;
    BatchOutputCollector _collector;
    IPartitionedBackingStore _store;
    
    public StatefulTransactionalBoltExecutor(IStatefulTransactionalBolt<T> delegate, IPartitionedBackingStore store) {
        _delegate = delegate;
        _store = store;
    }
    ...
    public void prepare(Map conf, TopologyContext context, BatchOutputCollector collector, TransactionAttempt attempt) {
        _state = (T) context.getTaskData(STATE_RESOURCE);
        if(_state == null) {
